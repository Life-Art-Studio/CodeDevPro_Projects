...
const Dashboard = () => {
  ...
  const leaderboard = users.filter(u => u.role === 'SALES').map(user => {
    const userOrders = orders.filter(o => o.createdBy === user.id && (o.status === 'Paid' || o.status === 'Delivered' || o.status === 'Partially Paid'));
              ...
              <div className="flex gap-4 min-w-max">
                {leaderboard.map((user, idx) => (
                  <div 
                    ...
                    onClick={() => {
                      if (currentUser?.role === 'ADMIN') {
                        logAction("Impersonate", "Dashboard", `Started viewing dashboard as ${user.name}`);
                  ...
                  >
                    ...
                    <h4 className="font-bold text-zinc-900 dark:text-zinc-100 truncate w-full">{user.name}</h4>
