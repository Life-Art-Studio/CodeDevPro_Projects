...
export const generateBeatPDF = (beat, customers, orders, visits, getOrderPaidAmount, getOrderOutstanding) => {
  ...
  doc.text(`Total Revenue: ₹${formatCurrency(totalRevenue)}`, rightMargin, sumY, { align: "right" });
  ...
  doc.text(`Avg Order Value: ₹${formatCurrency(averageOrderValue)}`, rightMargin, sumY + 18, { align: "right" });
  doc.text(`Outstanding: ₹${formatCurrency(outstandingBalance)}`, rightMargin, sumY + 24, { align: "right" });
