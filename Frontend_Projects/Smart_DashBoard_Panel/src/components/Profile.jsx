...
const ProfilePanel = () => {
  const { currentUser, isProfileOpen, onOpenProfileHandler, handleLogout, updateCurrentUser, users } = useAuth();

  // Safely grab user data for the display
  const userData = currentUser ?? { name: "Admin User", email: "admin@test.com" };
  ...
  const handleFileChange = (e) => {
    ...
    if (file) {
      if (file.size > 2 * 1024 * 1024) {
        toast.error("Image size should be less than 2MB to optimize storage.");
        return;
      }
  ...
  const save = () => {
    ...
    if (updateEmail !== userData.email) {
      const existing = users.find(u => u.email === updateEmail);
      if (existing) {
