    ...
    <div className="h-14 md:h-16 w-full bg-white dark:bg-[#0a0c14] backdrop-blur-2xl flex items-center justify-between px-4 md:px-6 border-b border-zinc-200 dark:border-zinc-800 shadow-sm relative z-10 transition-colors duration-500">
                    ...
                    {filteredOrders.map(order => (
                      ...
                      >
                        <div className="flex justify-between items-center">
                          ...
                          <span className={`text-[10px] px-2 py-0.5 rounded-full border ${order.status === 'Paid' ? 'bg-emerald-500/10 text-emerald-600 border-emerald-500/30 dark:text-emerald-400' :
      ...
      <div className="flex items-center gap-3 sm:gap-4 shrink-0">
        ...
        <div className="relative" ref={dropdownRef}>
          ...
          {/* Dropdown Card */}
          {isDropdownOpen && (
            <div className="absolute right-0 mt-2 w-56 bg-white dark:bg-[#1a1d27] shadow-xl rounded-2xl py-2 z-50 animate-slide-up-fade border border-zinc-100 dark:border-zinc-800">
              <div className="px-4 py-3 border-b border-zinc-200 dark:border-zinc-800">
                <div className="flex items-center justify-between mb-1">
                  <p className="text-sm font-semibold text-zinc-800 dark:text-zinc-100 truncate">{userData.name}</p>
                  {userData.role === 'ADMIN' ? (
                    <span className="text-[9px] font-bold uppercase tracking-wider px-1.5 py-0.5 rounded-md bg-indigo-100 text-indigo-700 dark:bg-indigo-500/20 dark:text-indigo-300">Admin</span>
                  ) : (
                    <span className="text-[9px] font-bold uppercase tracking-wider px-1.5 py-0.5 rounded-md bg-emerald-100 text-emerald-700 dark:bg-emerald-500/20 dark:text-emerald-300">
                      {userData.role ? userData.role.replace('_', ' ') : 'Sales Rep'}
                    </span>
                  )}
                </div>
                <p className="text-xs text-zinc-500 dark:text-zinc-400 truncate">{userData.email}</p>
                <div className="mt-2 text-[10px] text-zinc-400 dark:text-zinc-500">
                  {userData.role === 'ADMIN' ? (
                    <span>Organization Admin</span>
                  ) : connectedAdmin ? (
                    <span>Managed by: {connectedAdmin.name}</span>
                  ) : null}
                </div>
              </div>
