...
const ViewModeBanner = ({ viewAsUser }) => {
  ...
  const handleExit = () => {
    logAction("Impersonate", "Users", `Exited view mode for ${viewAsUser.full_name || viewAsUser.name || 'User'}`);
        ...
        <div className="flex items-center gap-2 min-w-0">
          {viewAsUser.profilePic ? (
            <img 
              ...
              alt={viewAsUser.full_name || viewAsUser.name || 'User'} 
            ...
            <div className="w-5 h-5 rounded-full bg-gradient-to-tr from-purple-500 to-pink-500 text-white flex items-center justify-center font-bold text-[9px] shadow-md ring-1 ring-white/10 shrink-0">
              {(viewAsUser.full_name || viewAsUser.name || 'U').charAt(0)}
            </div>
          )}
          <div className="flex items-center gap-1.5 min-w-0">
            <h3 className="font-bold text-[10px] sm:text-xs truncate whitespace-nowrap">Viewing as {viewAsUser.full_name || viewAsUser.name || 'User'}</h3>
