...
const Beats = () => {
  ...
  const { beats, addBeat, updateBeat, deleteBeat, addCustomerToBeat, removeCustomerFromBeat, isLoading } = useBeatContext();
  ...
  const renderCustomersTab = () => {
    ...
    return (
      <div className="flex flex-col h-full animate-in fade-in relative">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 h-full pb-16">
          ...
          <div className="flex flex-col">
            <div className="flex justify-between items-center mb-4">
              ...
              {currentUser?.role !== 'ADMIN' && selectedBeat?.owner_id === currentUser?.id && (
              ...
              {availableCustomers.length === 0 ? (
                ...
                availableCustomers.map(c => (
                  <label key={c.id} className={`flex items-center gap-3 p-3 rounded-xl border transition-colors ${
                    (currentUser?.role !== 'ADMIN' && selectedBeat?.owner_id === currentUser?.id) ? 'cursor-pointer' : 'cursor-default'
                  } ${
        ...
        {/* Floating Action Bar */}
        {selectedCustomers.size > 0 && currentUser?.role !== 'ADMIN' && selectedBeat?.owner_id === currentUser?.id && (
    ...
    return (
      <div className="flex-1 min-h-0 flex flex-col items-center justify-center bg-zinc-50 dark:bg-[#0f1117] transition-colors">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-zinc-500 dark:text-zinc-400 font-medium">Loading beats data...</p>
        </div>
      </div>
    );
  }
              ...
              <div className="mb-6">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                  ...
                  <div className="flex flex-wrap gap-2 w-full sm:w-auto">
                    ...
                    <button 
                      onClick={() => generateBeatPDF(selectedBeat, customers, orders, visits, getOrderPaidAmount, getOrderOutstanding, currentUser)}
