...
import { 
  ...
  ChevronRight, Sparkles, HelpCircle, Layers, CheckSquare, Square, Trash2
} from 'lucide-react';
...
const Catalogue = () => {
  ...
  const { products, addProduct, updateProduct, deleteProduct, deleteMultipleProducts, updateMultipleProducts, addMultipleProducts, replaceAISuggestions, useParentCatalogue, setUseParentCatalogue, isLoading } = useProductContext();
  
  // Steps: 'sector' | 'suggest' | 'catalogue' | 'form'
  const [step, setStep] = useState(() => (products.length > 0 ? 'catalogue' : 'sector'));

  // Update step if products are fetched and step was stuck at 'sector'
  useEffect(() => {
    if (!isLoading && products.length > 0 && step === 'sector') {
      setStep('catalogue');
    }
  }, [isLoading, products.length, step]);
  ...
  const handleSelectAllProducts = () => {
    if (selectedProductIds.size === finalFilteredCatalogue.length) {
      setSelectedProductIds(new Set());
    } else {
      setSelectedProductIds(new Set(finalFilteredCatalogue.map(p => p.id)));
    }
  };
  ...
  const handleBulkDelete = () => {
    ...
    toast((t) => (
      <div className="flex flex-col gap-2">
        <span className="font-semibold text-sm">Delete {selectedProductIds.size} selected products?</span>
        <div className="flex gap-2 justify-end mt-2">
          <button 
            onClick={async () => {
              toast.dismiss(t.id);
  ...
  const handleAutoCategorize = async () => {
    if (selectedProductIds.size === 0) return;
    const sector = SECTORS.find(s => s.id === (products[0]?.sector || "FMCG")); // fallback to FMCG or first product's sector
    if (!sector) return;
    
    const itemsToCategorize = products.filter(p => selectedProductIds.has(p.id));
    setIsCategorizing(true);
    const loadingToast = toast.loading(`AI is categorizing ${itemsToCategorize.length} products...`);
    
    try {
      const aiResults = await AIService.autoCategorizeProducts(itemsToCategorize, sector.id, sector.subCategories);
      
      const updatedProducts = itemsToCategorize.map(p => {
        const aiMatch = aiResults.find(r => r.id === p.id);
        if (aiMatch && aiMatch.suggestedCategory) {
          return { ...p, category: aiMatch.suggestedCategory, subCategory: aiMatch.suggestedCategory };
        }
        return p;
      });

      await updateMultipleProducts(updatedProducts);
      toast.success(`Successfully categorized ${itemsToCategorize.length} products.`);
      setSelectedProductIds(new Set());
    } catch (err) {
      toast.error(err.message || "Failed to auto-categorize products.");
    } finally {
    ...
    return (
      <div className="flex-1 min-h-0 flex flex-col items-center justify-center bg-zinc-50 dark:bg-[#0f1117] transition-colors">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-zinc-500 dark:text-zinc-400 font-medium">Loading catalogue...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="flex-1 min-h-0 flex flex-col z-0 relative w-full bg-zinc-50 dark:bg-[#0f1117] transition-colors duration-300">
      ...
      {/* Bulk Actions Bar (Global overlay) */}
      {step === 'catalogue' && selectedProductIds.size > 0 && (
        <div className="absolute bottom-6 left-1/2 -translate-x-1/2 z-[9999] bg-white dark:bg-[#1a1d27] border border-zinc-200 dark:border-zinc-700 shadow-2xl rounded-full px-6 py-4 flex items-center gap-4 animate-in slide-in-from-bottom-5 text-sm">
          ...
          >
            {selectedProductIds.size === finalFilteredCatalogue.length ? 'Deselect All' : 'Select All'}
          </button>
          <div className="h-6 w-px bg-zinc-200 dark:bg-zinc-700"></div>
          <button
            onClick={handleAutoCategorize}
            disabled={isCategorizing}
            className="flex items-center gap-1.5 text-xs font-medium text-purple-600 hover:text-purple-700 dark:text-purple-400 dark:hover:text-purple-300 transition-colors disabled:opacity-50"
          >
          ...
          <button
            ...
            className="flex items-center gap-1.5 text-xs font-medium text-red-600 hover:text-red-700 dark:text-red-400 dark:hover:text-red-300 transition-colors ml-2"
          >
      ...
      {step === 'suggest' && (
        <div className="flex-1 flex flex-col overflow-hidden relative">
          <div className="flex-1 overflow-y-auto p-4 sm:p-6 lg:p-8 custom-scrollbar">
            <div className="max-w-7xl mx-auto flex flex-col gap-6 animate-in slide-in-from-bottom-6 duration-400">
          ...
          {/* Bulk Add button (Sticky Footer) */}
          <div className="bg-white dark:bg-[#1a1d27] border-t border-zinc-200 dark:border-zinc-800 p-4 sm:px-6 lg:px-8 shrink-0 z-10 shadow-[0_-4px_20px_rgba(0,0,0,0.05)] dark:shadow-[0_-4px_20px_rgba(0,0,0,0.2)]">
                  ...
                  <div className="mt-3 flex items-center gap-3">
                    <label className="flex items-center cursor-pointer">
                      <div className="relative">
                        <input type="checkbox" className="sr-only" checked={useParentCatalogue} onChange={(e) => setUseParentCatalogue(e.target.checked)} />
                        <div className={`block w-10 h-6 rounded-full transition-colors ${useParentCatalogue ? 'bg-indigo-600' : 'bg-zinc-300 dark:bg-zinc-700'}`}></div>
                        <div className={`dot absolute left-1 top-1 bg-white w-4 h-4 rounded-full transition-transform ${useParentCatalogue ? 'transform translate-x-4' : ''}`}></div>
                      </div>
                      <span className="ml-3 text-sm font-semibold text-zinc-700 dark:text-zinc-300">Use Parent's Catalogue</span>
                    </label>
                  </div>
                )}
              </div>
          ...
          <div className="max-w-6xl mx-auto h-full">
            <ProductForm 
              ...
              availableCategories={[...new Set([
                ...(SECTORS.find(s => s.id === selectedSector)?.subCategories || []),
