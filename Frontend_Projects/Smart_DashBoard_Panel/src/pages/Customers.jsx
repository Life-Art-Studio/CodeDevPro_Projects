...
const Customers = () => {
  const { currentUser, users, setViewAsUserId } = useAuth();
  const { customers, addCustomer, deleteCustomer, updateCustomer, isLoading } = useCustomerContext();
  ...
  const displayCustomers = pendingFilterActive
    ? customers.filter((c) => safeOrders.some(o => (o.customerId || o.customer_id) === c.id && (o.status === "Pending" || o.status === "Partially Paid")))
    : inactiveFilterActive 
    ? customers.filter(c => {
        const custOrders = safeOrders.filter(o => (o.customerId || o.customer_id) === c.id);
  ...
  const customerMetrics = useMemo(() => {
    return customers.reduce((acc, customer) => {
      const custOrders = safeOrders.filter(o => (o.customerId || o.customer_id) === customer.id);
  ...
  const rowData = paginatedCustomers.map(customer => {
    const creator = users?.find(u => u.id === customer.owner_id);
    let db = null;
    if (creator) {
      if (creator.role === 'DISTRIBUTOR') db = creator;
      else db = users?.find(u => (creator.ancestor_ids?.includes(u.id) || creator.parent_id === u.id) && u.role === 'DISTRIBUTOR');
    }
    ...
    return {
            ...
            <div className="flex gap-2 mt-1.5">
              <span 
                onClick={(e) => {
                  e.stopPropagation();
                  if (creator) {
                    setViewAsUserId(creator.id);
                    toast.success(`Now viewing as ${creator.full_name}`);
                  }
                }}
                className={`text-[10px] px-1.5 py-0.5 rounded border ${creator ? 'cursor-pointer bg-zinc-100 hover:bg-zinc-200 dark:bg-zinc-800 dark:hover:bg-zinc-700 text-zinc-700 dark:text-zinc-300 border-zinc-300 dark:border-zinc-600' : 'bg-zinc-50 dark:bg-zinc-900 text-zinc-500 border-zinc-200 dark:border-zinc-800'}`}
              >
                Created by: {creator ? creator.full_name : 'Unknown'}
              </span>
              {db && (
                <span 
                  onClick={(e) => {
                    e.stopPropagation();
                    setViewAsUserId(db.id);
                    toast.success(`Now viewing as ${db.full_name}`);
                  }}
                  className="text-[10px] bg-indigo-50 hover:bg-indigo-100 dark:bg-indigo-500/10 dark:hover:bg-indigo-500/20 text-indigo-700 dark:text-indigo-300 px-1.5 py-0.5 rounded border border-indigo-200 dark:border-indigo-500/30 cursor-pointer transition-colors"
                >
                  DB: {db.full_name}
                </span>
              )}
      ...
      actions: (
        <div className="flex justify-end items-center gap-1 lg:gap-2">
          {currentUser?.role !== 'ADMIN' && customer.owner_id === currentUser?.id && (
            <>
              <button
              ...
              <div>
                ...
                <div className="flex gap-1.5 mt-1 flex-wrap">
                  <span 
                    ...
                    className={`text-[9px] px-1.5 py-0.5 rounded border ${creator ? 'cursor-pointer bg-zinc-100 hover:bg-zinc-200 dark:bg-zinc-800 dark:hover:bg-zinc-700 text-zinc-700 dark:text-zinc-300 border-zinc-300 dark:border-zinc-600' : 'bg-zinc-50 dark:bg-zinc-900 text-zinc-500 border-zinc-200 dark:border-zinc-800'}`}
                  >
                    Creator: {creator ? creator.full_name : 'Unknown'}
                  </span>
                  {db && (
                    <span 
                      onClick={(e) => {
                      ...
                      className="text-[9px] bg-indigo-50 hover:bg-indigo-100 dark:bg-indigo-500/10 dark:hover:bg-indigo-500/20 text-indigo-700 dark:text-indigo-300 px-1.5 py-0.5 rounded border border-indigo-200 dark:border-indigo-500/30 cursor-pointer transition-colors"
                    >
                      DB: {db.full_name}
                    </span>
                  )}
      ...
      {viewingCustomer ? (
        ...
        <div className="flex-1 min-h-0 flex flex-col items-center justify-center bg-zinc-50 dark:bg-[#0f1117] transition-colors">
          <div className="flex flex-col items-center gap-4">
            ...
            <p className="text-zinc-500 dark:text-zinc-400 font-medium">Loading customers...</p>
          </div>
        </div>
      ) : (
        <div className="flex-1 min-h-0 flex flex-col z-0">
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            ...
            <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto mt-2 sm:mt-0">
              ...
              {currentUser?.role !== 'ADMIN' && currentUser?.role !== 'SUPER_STOCKIST' && (
                <button
