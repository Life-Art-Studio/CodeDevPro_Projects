...
const CustomerDetail = ({ customer, onBack, defaultOrderId, defaultOrderFilter }) => {
  ...
  const { restockProduct, distributors, superStockists } = useSupplyChainContext();
  ...
  const customerVisits = visits.filter(v => (v.customer_id || v.customerId) === customer.id)
    .sort((a, b) => new Date(b.visit_date || b.visitDate) - new Date(a.visit_date || a.visitDate));

  useEffect(() => {
    if (defaultOrderId) {
      const orderToEdit = orders.find(o => o.id === defaultOrderId && (o.customerId === customer.id || o.customer_id === customer.id));
      if (orderToEdit) {
  ...
  const handleCreateNewOrder = () => {
    setActiveOrder({
      ...
      globalDiscount: "",
      items: [
        { id: Date.now(), name: "", qty: 1, price: "", discount: "", gst: 0 },
      ],
    });
    setOrderView("edit");
  };
  ...
  const handleSaveOrder = async (completedOrder) => {
    ...
    if (isExisting) {
      ...
      // Deduct inventory for the distributor when an order is placed
      if (restockProduct) {
        completedOrder.items.forEach(item => {
          const sku = item.sku || (item.name?.toLowerCase().includes("fluid") ? "BRK-FL-500" : "LUB-10W30-1L");
          const distributorId = completedOrder.distributor_id || completedOrder.distributorId || null;
          if (distributorId) restockProduct(distributorId, sku, -Number(item.qty));
        });
      }
    }
    setOrderView("list");
        ...
        <div className="flex gap-2 justify-end mt-2">
          <button 
            onClick={() => {
              ...
              if (orderToDelete && restockProduct) {
                orderToDelete.items.forEach(item => {
                  const sku = item.sku || (item.name?.toLowerCase().includes("fluid") ? "BRK-FL-500" : "LUB-10W30-1L");
                  const distributorId = orderToDelete.distributor_id || orderToDelete.distributorId || null;
                  if (distributorId) restockProduct(distributorId, sku, Number(item.qty));
                });
              }
              deleteOrder(orderId);
  ...
  const handleStatusChange = (orderId, newStatus, mode) => {
    ...
    if (orderToUpdate) {
      ...
      if (newStatus === 'Paid') {
        ...
        const total = calculateOrderTotal(orderToUpdate);
        const paid = getOrderPaidAmount(orderToUpdate);
        if (total > paid) {
          updatedOrder.paidAmount = total;
          updatedOrder.payments = [
            ...(orderToUpdate.payments || []),
            {
              ...
              amount: total - paid,
              method: mode || 'Cash',
              note: 'Auto-generated payment on Paid status',
          ...
          <div className="flex-1 overflow-y-auto space-y-4 custom-scrollbar pr-2 pb-10">
            {customerVisits.length === 0 ? (
              ...
              customerVisits.map(visit => {
                const beat = beats.find(b => b.id === (visit.beat_id || visit.beatId));
                return (
                    ...
                    <div className="flex justify-between items-start mb-2 pr-12">
                      <div>
                        <h4 className="font-bold text-zinc-900 dark:text-zinc-100 flex items-center gap-2">
                           ...
                           {visit.visit_date || visit.visitDate} • {visit.visit_time || visit.visitTime}
                        </h4>
